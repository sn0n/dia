import { scrapedLinearData, scrapedNonLinearData, scrapedDiagrams } from './scraped-diagrams'

export const complexWorkflows = {
  // Merge scraped diagrams with code
  ...scrapedDiagrams,
  // Manual complex workflows below
...
export const rawNonLinearData = [
  ...
  "UML Class Diagram|Define Classes → Add Attributes → Add Methods → Define Relationships → Inheritance|Diagrams",
  "Sequence Diagram|Actor A → Message → Actor B → Response → Actor A → Complete|Diagrams",
  "Entity Relationship|Define Entities → Identify Attributes → Define Relationships → Cardinality → Keys|Diagrams",
  "State Machine|Initial State → Event → Transition → New State → Guard Condition → Final State|Diagrams",
  "Use Case Diagram|Actor → Use Case → System Boundary → Include/Extend → Relationships|Diagrams",
  "Component Diagram|Components → Interfaces → Dependencies → Ports → Connectors|Diagrams",
  "Deployment Diagram|Nodes → Artifacts → Deploy → Communication Paths → Protocols|Diagrams",
  "Activity Diagram|Start → Action → Decision → Fork/Join → Merge → End|Diagrams",
  "Network Topology|Nodes → Edges → Layout → Centrality → Clustering → Paths|Diagrams",
  "Mind Map|Central Idea → Branch → Sub-branch → Leaf → Connect → Organize|Diagrams",
  "Sankey Diagram|Source → Flow → Intermediate → Split/Merge → Destination → Quantify|Diagrams",
  "Sunburst Chart|Root → Ring 1 → Ring 2 → Ring 3 → Hierarchical → Angular|Diagrams",
  "Treemap|Root → Partition → Rectangles → Hierarchy → Size → Color|Diagrams",
  "Chord Diagram|Entities → Relationships → Arcs → Ribbons → Bidirectional|Diagrams",
  "Force-Directed Graph|Nodes → Links → Physics → Attraction → Repulsion → Equilibrium|Diagrams",
  "Timeline|Events → Chronological → Milestones → Duration → Spans → Now|Diagrams",
  "Gantt Chart|Tasks → Dependencies → Start Date → Duration → Progress → Resources|Diagrams",
  "Fishbone (Ishikawa)|Problem → Categories → Causes → Sub-causes → Root Cause → Action|Diagrams",
  "Venn Diagram|Set A → Set B → Intersection → Union → Complement → Visualize|Diagrams",
  "Flowchart Symbols|Process → Decision → Data → Terminal → Connector → Document|Diagrams",
  "BPMN Process|Start Event → Task → Gateway → Intermediate Event → End Event → Pool/Lane|Diagrams",
  "Data Flow Diagram|External Entity → Process → Data Store → Data Flow → Context → Levels|Diagrams",
  "Petri Net|Places → Transitions → Tokens → Arcs → Fire → Marking|Diagrams",
  "Signal Flow|Input → Block → Transfer Function → Summing Junction → Output → Feedback|Diagrams",
  "Circuit Diagram|Power → Components → Connections → Ground → Signals → Schematic|Diagrams",
  "Logic Gate|Input A → Gate (AND/OR/NOT) → Output → Truth Table → Circuit|Diagrams",
  "Timing Diagram|Clock → Signal 1 → Signal 2 → Timing → Propagation Delay → Cycle|Diagrams",
  "Wireframe|Layout → Components → Navigation → Content → Interaction → Annotations|Diagrams",
  "User Journey Map|Persona → Touchpoint → Action → Emotion → Pain Point → Opportunity|Diagrams",
  "Service Blueprint|Customer Actions → Frontstage → Backstage → Support → Evidence|Diagrams",
  "Sitemap|Homepage → Section → Subsection → Pages → Hierarchy → Navigation|Diagrams",
  "C4 Model Context|System → Users → External Systems → Relationships → Boundaries|Diagrams",
  "C4 Container|Containers → Technology → Interactions → Database → APIs|Diagrams",
  "C4 Component|Components → Responsibilities → Dependencies → Interfaces|Diagrams",
  "Layered Architecture|Presentation → Business Logic → Data Access → Database → Separation|Diagrams",
  "Microservices Map|Services → API Gateway → Communication → Data Stores → Load Balancer|Diagrams",
  "Infrastructure Diagram|Servers → Network → Storage → Security → Monitoring → Backup|Diagrams",
  "Cloud Architecture|Region → VPC → Subnets → EC2 → RDS → S3 → CloudFront|Diagrams",
  "Kubernetes Architecture|Cluster → Nodes → Pods → Services → Ingress → Volumes|Diagrams",
  "CI/CD Pipeline|Code → Build → Test → Stage → Deploy → Monitor → Feedback|Diagrams",
  "GitOps Workflow|Git Commit → Automation → Sync → Deploy → Observe → Reconcile|Diagrams",
  "Decision Table|Conditions → Rules → Actions → Outcomes → Completeness|Diagrams",
  "Heat Map|Data Matrix → Color Scale → Patterns → Clusters → Outliers|Diagrams",
  "Radar Chart|Axes → Values → Area → Comparison → Multi-dimensional|Diagrams",
  "Bubble Chart|X-Axis → Y-Axis → Size → Color → Quadrants → Trends|Diagrams",
  "Box Plot|Min → Q1 → Median → Q3 → Max → Outliers → Distribution|Diagrams",
  "Waterfall Chart|Starting Value → Increases → Decreases → Cumulative → Ending Value|Diagrams",
  "Funnel Chart|Stage 1 → Stage 2 → Stage 3 → Conversion → Drop-off → Optimize|Diagrams",
  "Org Chart|CEO → Departments → Teams → Managers → Individual Contributors|Diagrams",
  "Family Tree|Ancestors → Parents → Siblings → Children → Descendants → Relations|Diagrams",
  "Dependency Graph|Nodes → Dependencies → Transitive → Cycles → Order → Resolve|Diagrams",
  "Parse Tree|Root → Non-terminals → Terminals → Grammar → Derivation|Diagrams",
  "Abstract Syntax Tree|Program → Declarations → Statements → Expressions → Leaves|Diagrams",
  "Memory Map|Stack → Heap → Code → Data → BSS → Memory Layout|Diagrams",
  "Thread Interaction|Thread 1 → Shared Resource → Lock → Thread 2 → Unlock → Synchronize|Diagrams",
  "Database Schema|Tables → Columns → Types → Constraints → Indexes → Foreign Keys|Diagrams",
  "Event Storming|Domain Event → Command → Aggregate → Policy → Read Model|Diagrams",
  "Value Stream|Supplier → Process → Value Add → Wait → Customer → Lead Time|Diagrams",
  "Capability Map|Business Capabilities → Sub-capabilities → Hierarchy → Dependencies|Diagrams",
  "Impact Mapping|Goal → Actor → Impact → Deliverable → Prioritize|Diagrams",
  "Story Mapping|User Activities → Tasks → Stories → Releases → Backbone|Diagrams",
  "Ecosystem Map|Actors → Value Exchanges → Relationships → Network → Dynamics|Diagrams",
  "Wardley Map|Components → Evolution → Value Chain → Movement → Strategy|Diagrams",
  "Risk Matrix|Likelihood → Impact → Severity → Mitigation → Priority|Diagrams",
  "Threat Model|Assets → Threats → Vulnerabilities → Countermeasures → Risk|Diagrams",
  "Attack Tree|Goal → OR Gate → AND Gate → Leaf Attacks → Probability → Cost|Diagrams",
  "Probability Tree|Event → Outcomes → Probabilities → Branches → Calculate|Diagrams",
  "Causal Loop|Variable A → Influence → Variable B → Feedback → Loop → Delay|Diagrams",
  "Stock and Flow|Stock → Inflow → Outflow → Accumulation → Depletion → Balance|Diagrams",
  "Concept Map|Concept → Link → Relationship → Proposition → Hierarchy|Diagrams",
  "Affinity Diagram|Ideas → Group → Category → Organize → Patterns → Insights|Diagrams",
  "Matrix Diagram|Row Items → Column Items → Intersections → Relationships → Strength|Diagrams",
  "Process Map|Start → Steps → Decision Points → Handoffs → End → Metrics|Diagrams",
  "Swimlane Diagram|Actors → Lanes → Process Steps → Handoffs → Flow → Responsibilities|Diagrams",
  "Value Chain|Primary Activities → Support Activities → Value → Margin → Competitive Advantage|Diagrams",
  "Porter's Value Chain|Inbound → Operations → Outbound → Marketing → Service → Margin|Diagrams",
  "Business Model Canvas|Partners → Activities → Resources → Value → Customers → Costs → Revenue|Diagrams",
  "Lean Canvas|Problem → Solution → Metrics → Value → Channels → Segments → Costs → Revenue|Diagrams",
  "Empathy Map|Says → Thinks → Does → Feels → Pains → Gains → Insights|Diagrams",
  "Persona Canvas|Demographics → Goals → Frustrations → Motivations → Behaviors → Needs|Diagrams",
  "Jobs-to-be-Done|Job → Situation → Motivation → Outcome → Success Criteria|Diagrams",
  "Feature Comparison|Features → Product A → Product B → Product C → Rating → Decision|Diagrams",
  "Pros and Cons|Option → Pros → Cons → Weight → Score → Choose|Diagrams",
  "Decision Matrix|Options → Criteria → Weights → Scores → Total → Rank|Diagrams",
  "Pugh Matrix|Baseline → Alternatives → Criteria → Better/Same/Worse → Sum → Select|Diagrams",
  "Morphological Chart|Functions → Alternatives → Combinations → Solutions → Evaluate|Diagrams",
  "TRIZ Contradiction Matrix|Worsening → Improving → Principles → Solutions|Diagrams",
  "Root Cause Tree|Problem → Why? → Cause → Why? → Root Cause → Action|Diagrams",
  "Event Chain|Trigger → Event 1 → Event 2 → Event 3 → Outcome → Prevent|Diagrams",
  "Influence Diagram|Decision → Uncertainty → Value → Dependencies → Optimize|Diagrams",
  "System Diagram|Inputs → Process → Outputs → Feedback → Environment|Diagrams",
  "Context Diagram|System → External Entities → Data Flows → Boundary|Diagrams",
  "Hexagonal Architecture|Domain → Ports → Adapters → Infrastructure → Isolation|Diagrams",
  "Onion Architecture|Domain Model → Services → Infrastructure → UI → Dependencies Inward|Diagrams",
  "Event-Driven Architecture|Event → Producer → Broker → Consumer → Process → Store|Diagrams",
  "CQRS Pattern|Command → Write Model → Event → Read Model → Query → View|Diagrams",
  "Saga Pattern|Service 1 → Success → Service 2 → Failure → Compensate → Rollback|Diagrams",
  "API Gateway Pattern|Client → Gateway → Routing → Services → Response → Aggregate|Diagrams",
  "Load Balancer|Requests → Distributor → Server 1/2/3 → Health Check → Response|Diagrams",
  ...scrapedNonLinearData
];
